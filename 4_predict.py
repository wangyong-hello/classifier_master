...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=r'C:\Users\14833\Desktop\archive\Testing', help='source data path(file, folder)')
    ...
    parser.add_argument('--model', type=str, default='resnet50_3064增强大数据集(laeblsoooth0.1)', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')

    opt = parser.parse_known_args()[0]
    
    train_model_save_path=opt.save_path+'/train/'+ opt.model
    ...
    print('found checkpoint from {}, model type:{}\n{}'.format(train_model_save_path, ckpt['model'].name, dict_to_PrettyTable(ckpt['best_metrice'], 'Best Metrice')))
...
if __name__ == '__main__':
    ...
    if os.path.isdir(opt.source):
        ...
        result = []
        for file in tqdm.tqdm(os.listdir(opt.source)[0:]):
