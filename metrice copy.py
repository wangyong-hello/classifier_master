...
def parse_opt():
    ...
    parser.add_argument('--weight_path', type=str, default=r'runs/exp/train', help='save path for model and log')  #tag:存放模型文件的路径
    ...
    if not os.path.exists(os.path.join(opt.weight_path, 'best.pt')):
        raise Exception('best.pt not found. please check your --weight_path folder')
    ckpt = torch.load(os.path.join(opt.weight_path+'\\resnet50', 'best.pt'))
    train_opt = ckpt['opt']

    set_seed(train_opt.random_seed)
    model = Model_Inference(DEVICE, opt)

    print('found checkpoint from {}, model type:{}\n{}'.format(opt.weight_path, 'resnet50', dict_to_PrettyTable(ckpt['best_metrice'], 'Best Metrice')))
    # print('found checkpoint from {}, model type:{}\n{}'.format(opt.weight_path, ckpt['model'].name, dict_to_PrettyTable(ckpt['best_metrice'], 'Best Metrice')))

    test_transform = utils_aug.get_dataprocessing_teststage(train_opt, opt, torch.load(os.path.join(opt.weight_path, 'preprocess.transforms')))
