...
from utils.utils_fit_cuxi import fitting
...
from utils.utils_dataset_cuxi import Dataset
...
def parse_opt():
    ...
    parser.add_argument('--model_ater_name', type=str, default='resnet50_PReLU_aug_mean_acc', help='用来做保存的文件夹名')
    ...
    parser.add_argument('--train_list', type=str, default=r'.\Datasets\label_7023_brian_val_CUXI.csv', help='train data path')
    parser.add_argument('--val_list', type=str, default=r'.\Datasets\label_7023_brian_val_CUXI.csv', help='val data root')
    parser.add_argument('--train_path', type=str, default=r'C:\Users\14833\Desktop\process_dataset\dataset', help='train data path，用来计算mean，std')   
    ...
    if opt.resume:
        ...
        # model = load_weights(model, opt).to(DEVICE)
            ...
            model = models.resnet50( pretrained=False)   #tag:模型的定义
            ...
            model.coarse_classifier =torch.nn.Sequential(
                torch.nn.Dropout(0.2),
                torch.nn.Linear(model.coarse_classifier.in_features, 1),
                # torch.nn.ReLU(),
                # torch.nn.Linear(1, 1),
                # torch.nn.Sigmoid()
            )
            model.fine_classifier =torch.nn.Sequential(
                torch.nn.Dropout(0.2),
                torch.nn.Linear(model.fine_classifier.in_features, 3),
                # torch.nn.ReLU(),
                # torch.nn.Linear(3, 3),
                # torch.nn.Softmax()
            )
    ...
    if opt.resume:
        ...
        # loss=coarse_classifier_loss +  fine_classifier_loss

        if opt.rdrop:
            loss = RDropLoss(loss)
    return opt, model, trainloader, testloader, optimizer, scaler, lr_scheduler, coarse_classifier_loss,fine_classifier_loss ,DEVICE, CLASS_NUM, (
        ckpt['epoch'] if opt.resume else 0), (ckpt['best_metrice'] if opt.resume else None)


if __name__ == '__main__':
    opt, model, train_dataset, test_dataset, optimizer, scaler, lr_scheduler,coarse_classifier_loss, fine_classifier_loss, DEVICE, CLASS_NUM, begin_epoch, best_metrice = parse_opt()
    ...
    for epoch in range(begin_epoch, opt.epoch):
        ...
        metrice = fitting(model,coarse_classifier_loss, fine_classifier_loss, optimizer, train_dataset, test_dataset, CLASS_NUM, DEVICE, scaler,'{}/{}'.format(epoch + 1, opt.epoch), opt)
        '''
